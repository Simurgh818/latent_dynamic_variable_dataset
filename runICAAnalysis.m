function [R2_test, MSE_test, outICA] = runICAAnalysis(eeg_train, eeg_test, h_train, h_test, num_comps, param, fs_new, results_dir)
...
%   fs_new              : Sampling rate for FFT plotting
%   results_dir         : Parent folder for results
...
EEG.trials = 1;
EEG.srate  = fs_new; % Use fs_new if that's the current data rate
EEG.xmin   = 0;
...
try
    EEG = pop_runica(EEG, 'extended', 1, 'pca', num_comps, 'interrupt','off', 'verbose', 'off');
catch
    % Fallback if extended infomax fails or toolbox issues
    warning('Extended ICA failed or not found, trying "runica" default');
    EEG = pop_runica(EEG, 'pca', num_comps, 'interrupt','off', 'verbose', 'off');
end
...
for f = 1:param.N_F
    c = corrcoef(h_train(:,f), h_rec_train(:,f));
...
for f=1:param.N_F
    ...
    % Plot True
    plot(h_train(:, f),'LineStyle', '-', 'Color', h_f_colors(f, :),'DisplayName', ['$Z_{' num2str(f) '}$ (t) ']);
    % Plot Recon
    plot(h_rec_train(:, f), 'LineStyle', '--','Color', 'k','DisplayName', ['$\hat{Z}_{' num2str(f) '}$ (t) ']);
    
    ylabel('Amp');
    xlim([0 fs_new*2]); % 2 seconds snapshot
    legend('Show','Interpreter', 'latex', 'Location','eastoutside');
    
    rho = zeroLagCorr_ica(f);
    text(0.02 * fs_new, 0.7 * max(abs(h_train(:,f))), ...
...
outICA.MSE_train    = mean((h_train - h_rec_train).^2, 'all');
outICA.results_dir  = method_dir;

% Save Summary Plots to Main Directory
saveas(fig1, fullfile(results_dir, ['Main_Summary_Trace_ICA' file_suffix '.png']));
